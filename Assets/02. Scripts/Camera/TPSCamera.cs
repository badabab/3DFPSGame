...
// 3인칭 슈팅: Third Person Shooting
// 게임상의 캐릭터가 보는 시점이 아닌, 캐릭터가 보는 시점 즉, 3인칭 관찰자 시점의 카메라
public class TPSCamera : MonoBehaviour
{
    // ** 카메라 회전 **
    // 목표: 마우스를 조작하면 카메라를 캐릭터 중심에 따라 회전시키고 싶다.
    // 필요 속성:
    // - 회전 속도
    public float RotationSpeed = 200;
    // - 타겟(캐릭터)
    public Transform Target;
    public Vector3 Offset = new Vector3(-5f, 3f, 0); // 오프셋 (0, 3f, -3f)
    // - 누적할 x각도와 y각도
    private float _mx = 0;
    private float _my = 0;
